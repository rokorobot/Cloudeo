...
class TregCLIClient(TregClient):
    ...
    async def discover(
        ...
        if not isinstance(rows, list):
            raise TregError(
                "Treg catalog search returned invalid results."
            )

        # Cloudeo, not Treg, is making the provider decision.
        concrete = [
            row
            for row in rows
            if isinstance(row, dict)
            and row.get("id")
            and row.get("kind") != "routed"
        ]

        # Search can match adjacent jobs. Anchor to the highest-ranked
        # concrete capability, then compare providers for THAT job.
        capability = next(
            (
                row.get("capability")
                for row in concrete
