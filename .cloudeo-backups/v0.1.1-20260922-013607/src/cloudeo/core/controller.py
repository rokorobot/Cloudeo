...
class Controller:
    ...
    async def run(self, request: RunRequest) -> RunResponse:
        ...
        criteria["escalate"] = (
            "None of the candidate tools is appropriate or "
            "there is not enough information to choose safely."
        )
            ...
            routing_state,
            {
                "tool_route": {
                    ...
                    "instructions": (
                        ...
                        "Prefer a tool that can directly satisfy the objective "
                        "with the supplied state. Choose escalate if none fit."
                    ),
                    "criteria": criteria,
                }
            },
        ...
        probabilities = {
            str(k): float(v)
            for k, v in (
                route_answer.get("probabilities") or {}
            ).items()
        }

        choice = str(
            route_answer.get("choice", "escalate")
        )

        confidence = float(
            route_answer.get("confidence") or 0.0
        )

        if (
            choice == "escalate"
            or confidence < route_threshold
        ):
            response = RunResponse(
                ...
                reason=(
                    ...
                    if choice == "escalate"
                        ...
                        f"Routing confidence {confidence:.3f} "
                        f"is below threshold {route_threshold:.3f}."
                    )
                ),
            )
        ...
        for tool_id in ranked[:max_attempts]:
            ...
            try:
                output = await self.treg.execute(
                    candidate,
            ...
            if deterministic.status == "pass":
                ...
                probability = float(
                    verification["answers"]["sufficient"].get(
                        "noul"
                    )
                    or 0.0
                )
            ...
            if passed:
                if (
                    ...
                    reason = (
                        "Tool result passed deterministic "
                        "validation; Jev verification was "
                        "unnecessary."
                    )
                else:
                    reason = (
                        f"Tool result verified by Jev at "
                        f"{probability:.3f}, meeting threshold "
        ...
        response = RunResponse(
            ...
            attempts=attempts,
            reason=(
                "No attempted tool produced a result "
